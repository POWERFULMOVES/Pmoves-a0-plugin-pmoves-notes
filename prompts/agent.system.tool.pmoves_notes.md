...
- `save_note`: args `content`, optional `title`, `tags` (list of strings)
- `search_notes`: args `query`, optional `limit` (default 10, max 50), `tags` (list of strings)

notes:
- `save_note` returns the new note id; `search_notes` returns matching notes with snippets
